...
def get_iou(det, gt):
    ...
    area_union = float(det_area + gt_area - area_intersection + 1e-6) #epsilon 
...
# ********** WILL UNDERSTAND THIS(LINE BY LINE) **MUST ****************
...
def compute_map(det_boxes, gt_boxes, iou_threshold = 0.5, method = 'area', difficult = None):
    ...
    for idx, label in enumerate(gt_labels):
        # Get detection predictiion for this class
        cls_dets = [
        ...
        for det_idx, (im_idx, det_pred) in enumerate(cls_dets):
            ...
            tp = np.cumsum(tp) # compute over flattened array 
            fp = np.cumsum(fp)
            
            eps = np.finfo(np.float32).eps   # gives machine epsilon eps = 1.1920929 x 10 ^-7
...
def infer(args):
    ...
    for i in tqdm(range(num_samples)):
        ...
        _, ssd_detections = model(im_tensor.squeeze(0).to(device), [target])
        ...
        cv2.addWeighted(src1=gt_im_copy, alpha=0.7, src2=gt_im, beta = 0.3, gamma = 0, dst = gt_im)
        cv2.imwrite('samples/output_ssd_gt{}.png'.format(i), gt_im)
        ...
        for idx, box in enumerate(boxes):
            ...
            # Scale to image dimensions
            x1, y1, x2, y2 = int(w * x1), int(h * y1), int(w * x2),  int(h * y2)
            cv2.rectangle(im, (x1,y1), (x2,y2), thickness=2, color = [0, 255, 0])
            cv2.rectangle(im_copy, (x1, y1), (x2, y2), thickness=2, color = [0, 255, 0])
            text = '{} : {:.2f}'.format(voc.label2idx[labels[idx].detach().cpu().item()], 
        ...
        cv2.addWeighted(src1=im_copy, alpha=0.7, src2=im, beta = 0.3, gamma = 0, dst = im)
        cv2.imwrite('samples/output_ssd_pred{}.png'.format(i), im)
...
def evaluate_map(args):
    ...
    for im_tensor, target, fname in tqdm(test_dataset):
        ...
        for label_name in voc.label2idx: # get class names
        ...
        for idx, box in enumerate(boxes):
            x1,y1, x2, y2 = box.detach().cpu().numpy()
            label = labels[idx].detach().cpu().item() # scalar value ; integer
        ...
        for idx, box in enumerate(target_bboxes):
            ...
            difficult_boxes[label_name].append(difficult_boxes[idx].detach().cpu().numpy())
    ...
    print("Mean Average Precision: {mean_ap:.4f}")      
